...
void AxisCanvas2D::updatePlotRect()
{
    ...
    newRect.setLeft(mrg->left() + m_yLabelMargin + m_yTicksMargin
                    + (m_yTicksWidth + m_yLabel.size().height())/m_textscaling);
    newRect.setTop(mrg->top() + m_titleMargin + m_title.size().height()/m_textscaling);
    newRect.setRight(m_axis->width() - mrg->right());
    newRect.setBottom(m_axis->height() - mrg->bottom() - m_xTicksMargin - m_xLabelMargin
                      - (m_xTicksHeight + m_xLabel.size().height())/m_textscaling);
...
QStaticText *AxisCanvas2D::getStaticText(const QString &text)
{
    if (m_tickTextPool.isEmpty()) {
        QStaticText *st = new QStaticText(text);
        return st;
    } else {
...
{
    QFont font = getFont(m_axis->titleFont());
    painter->setFont(font);

    qreal x = 0.5*(width() - m_title.size().width());
    qreal y = m_textscaling*m_axis->margin()->top();
    painter->drawStaticText(x, y, m_title);
}

void AxisCanvas2D::drawXLabel(QPainter *painter, const QRectF &plotRect)
{
    QFont font = getFont(m_axis->xAxis()->labelFont());
    painter->setFont(font);

    qreal x = m_textscaling*plotRect.center().x() - 0.5*m_xLabel.size().width();
    qreal y = plotRect.bottom() + m_xTicksMargin + m_xTicksHeight/m_textscaling + m_xLabelMargin;
    painter->drawStaticText(x, m_textscaling*y, m_xLabel);
}

void AxisCanvas2D::drawYLabel(QPainter *painter, const QRectF &plotRect)
{
    QFont font = getFont(m_axis->yAxis()->labelFont());
//    font.setPointSize(font.pointSize()*2);
    painter->setFont(font);

    qreal x = m_textscaling*m_axis->margin()->left();
    qreal y = m_textscaling*plotRect.center().y();

    QTransform tran = QTransform().translate(x,y).rotate(-90);
    m_yLabel.prepare(tran, font);
    painter->save();
...
//    m_yLabel.prepare(QTransform(), font);
//    QSize sz = 2.2*m_yLabel.size().toSize();
//    QPixmap pix(sz);
//    pix.fill(Qt::transparent);
//    QPainter pt(&pix);
//    pt.setRenderHints(QPainter::Antialiasing | QPainter::TextAntialiasing);
//    pt.setFont(font);
//    pt.drawText(pix.rect(), Qt::AlignHCenter | Qt::AlignVCenter, m_yLabel.text());
//    pt.end();
//    pix = pix.transformed(QTransform().rotate(-90));

//    painter->drawPixmap(x, y, 0.5*sz.height(), 0.5*sz.width(), pix);
}

void AxisCanvas2D::drawXTickLabels(QList<qreal> xFrames, QPainter *painter, const QRectF &plotRect)
{
    QFont font = getFont(m_axis->yAxis()->tickFont());
    painter->setFont(font);
    ...
    for (int i=0; i<xFrames.size(); ++i) {
        ...
        painter->drawStaticText(m_textscaling*x - sz.width()/2, m_textscaling*(y + m_xTicksMargin), *st);
    }
}
...
void AxisCanvas2D::drawYTickLabels(QList<qreal> yFrames, QPainter *painter, const QRectF &plotRect)
{
    ...
    for (int i=0; i<yFrames.size(); ++i) {
        ...
        painter->drawStaticText((x - m_yTicksMargin)*m_textscaling - sz.width(), m_textscaling*y - 0.5*sz.height(), *st);
    }
}
...
//    qreal minorSize = m_axis->xAxis()->minorTickSize();
    ...
    foreach (qreal x, major) {
        lines << m_textscaling*QPointF(x, y1);
        lines << m_textscaling*QPointF(x, y2);
    }
...
//    qreal minorSize = m_axis->yAxis()->minorTickSize();
    ...
    foreach (qreal y, major) {
        lines << m_textscaling*QPointF(x1, y);
        lines << m_textscaling*QPointF(x2, y);
    }
...
void AxisCanvas2D::drawScale(QPainter *painter, const QRectF &plotRect)
{
    ...
    qreal xScale_x = m_textscaling*plotRect.right() - m_xScaleText.size().width();
    qreal xScale_y = plotRect.bottom() + m_xTicksMargin + m_xTicksHeight/m_textscaling + m_xLabelMargin;
    painter->drawStaticText(xScale_x, m_textscaling*xScale_y, m_xScaleText);

    qreal yScale_x = m_textscaling*plotRect.left();
    qreal yScale_y = m_textscaling*plotRect.top() - m_yScaleText.size().height() - 2;
...
void AxisCanvas2D::drawGrid(QList<qreal> xMajor, QList<qreal> yMajor, QPainter *painter, const QRectF &plotRect)
{
    ...
    if (m_axis->grid()->axes() & AxisGrid::X) {
        foreach (qreal x, xMajor) {
            lines << m_textscaling*QPointF(x, plotRect.top());
            lines << m_textscaling*QPointF(x, plotRect.bottom());
        }
    }
    if (m_axis->grid()->axes() & AxisGrid::Y) {
        foreach (qreal y, yMajor) {
            lines << m_textscaling*QPointF(plotRect.left(), y);
            lines << m_textscaling*QPointF(plotRect.right(), y);
        }
    }
...
QFont AxisCanvas2D::getFont(const QFont &font)
{
    QFont newfont = font;
    newfont.setPointSize(newfont.pointSize() * m_textscaling);
    return newfont;
}
